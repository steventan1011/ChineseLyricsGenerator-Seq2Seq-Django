...
from backend.network import Model
import numpy as np
...
# FILE_PATH = "/content/drive/My Drive/Colab Notebooks/Lyricist-master"
...
def train(dataset, learning_rate, total_epoch, device=None, save_epoch=5, log_step=100, check_epoch=1):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(dataset.data_iter):        # 迭代
            ...
            temp_loss += loss.cpu().data.numpy()
            temp_acc += check(dataset, net, encoder_input, encoder_length, decoder_input, decoder_length, target)
            num += 1
            if step % log_step == 0:
                if step > 0:
        ...
        loss_list.append(temp_loss / num)
        acc_list.append(temp_acc / num)
        print(loss_list)
        print(acc_list)
    ...
    plt.show()
    plt.savefig("Validation accuracy_loss.jpg")
...
def check(dataset, net, encoder_input, encoder_length, decoder_input, decoder_length, target):
    ...
    acc = len([i for i in prediction if i in target]) * 100 / len(target)
